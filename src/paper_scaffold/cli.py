...
def command_recipes(args: argparse.Namespace) -> int:
    recipe_command = getattr(args, "recipe_command", "list")
    if recipe_command in {None, "list"}:
        print(list_recipes())
        return 0
    if recipe_command == "show":
        try:
        ...
        return 0
    print(f"Unknown recipes command: {recipe_command}")
    return 2
...
def command_demo(args: argparse.Namespace) -> int:
    ...
    demo_main_tex = r"""\documentclass[11pt]{article}
...
\title{Paper Scaffold Demo Manuscript}
...
\begin{abstract}
This synthetic manuscript demonstrates a clean paper repository with a copied figure, a small table, a bibliography entry, and artifact provenance.
\end{abstract}

\section{Example Result}

Figure~\ref{fig:example-metric} shows a synthetic result generated by the example Python workflow. Table~\ref{tab:example-summary} gives a tiny generated table. This example cites a placeholder reference \citep{example_reference} so bibliography checks have a complete citation path.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\linewidth]{figures/example_metric_plot.pdf}
\caption{Synthetic metric plot generated for the Paper Scaffold demo.}
\label{fig:example-metric}
\end{figure}

\begin{table}[htbp]
\centering
\input{tables/example_table.tex}
\caption{Tiny synthetic summary table generated for the Paper Scaffold demo.}
...
\end{document}
"""
    (output / "main.tex").write_text(demo_main_tex, encoding="utf-8")

    manifest = output / "metadata" / "artifact_manifest.yaml"
    candidates = [
        candidate
        for candidate in discover_artifacts(example_outputs)
        if candidate.source_path.name in {"example_metric_plot.pdf", "example_summary_table.csv", "example_table.tex"}
    ]
...
def command_audit_project(args: argparse.Namespace) -> int:
    try:
        result = audit_project(Path(args.path))
    except FileNotFoundError:
        print(f"Project path does not exist: {args.path}")
        return 2
    output = format_project_audit(result)
    print(output)
    if args.write_report:
        report_path = Path(args.write_report)
        report_path.parent.mkdir(parents=True, exist_ok=True)
        report_path.write_text(output, encoding="utf-8")
        print(f"Wrote project audit report: {report_path}")
    return 0


def command_release_check(args: argparse.Namespace) -> int:
    result = run_release_check(Path(args.manuscript_repo or Path.cwd()))
    output = format_release_check(result)
    print(output)
    if args.write_report:
        report_path = Path(args.write_report)
        report_path.parent.mkdir(parents=True, exist_ok=True)
        report_path.write_text(output, encoding="utf-8")
        print(f"Wrote release check report: {report_path}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    recipes.set_defaults(func=command_recipes, recipe_command="list")
    recipes_list = recipes_subparsers.add_parser("list", help="List available recipes")
    recipes_list.set_defaults(func=command_recipes, recipe_command="list")
    recipes_show = recipes_subparsers.add_parser("show", help="Show one recipe")
    recipes_show.add_argument("recipe_id")
    recipes_show.set_defaults(func=command_recipes, recipe_command="show")
    ...
    project_audit = subparsers.add_parser("audit-project", help="Read-only audit of a messy project folder")
    project_audit.add_argument("--path", required=True)
    project_audit.add_argument("--write-report", help="Write a Markdown project audit report")
    project_audit.set_defaults(func=command_audit_project)

    release_check = subparsers.add_parser("release-check", help="Run consolidated pre-submission manuscript checks")
